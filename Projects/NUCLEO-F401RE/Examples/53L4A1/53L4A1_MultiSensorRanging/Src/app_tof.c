...
{
  [VL53L4A1_DEV_LEFT] = "LEFT",
  [VL53L4A1_DEV_CENTER] = "CENTER",
  [VL53L4A1_DEV_RIGHT] = "RIGHT"
};

/* Private function prototypes -----------------------------------------------*/
static void MX_53L4A1_MultiSensorRanging_Init(void);
static void MX_53L4A1_MultiSensorRanging_Process(void);
...
static void MX_53L4A1_MultiSensorRanging_Init(void)
{
  ...
  printf("53L4A1 Multi Sensor Ranging demo application\n");
  ...
  for (device = 0; device < RANGING_SENSOR_INSTANCES_NBR; device++)
  {
    ...
    status = VL53L4A1_RANGING_SENSOR_Init(device);

    if (status != BSP_ERROR_NONE)
    {
      printf("VL53L4A1_RANGING_SENSOR_Init %d failed\n", device);
  ...
  for (device = 0; device < RANGING_SENSOR_INSTANCES_NBR; device++)
  {
    ...
    i2c_addr = (RANGING_SENSOR_VL53L4CD_ADDRESS + (device + 1) * 2);
    VL53L4A1_RANGING_SENSOR_SetAddress(device, i2c_addr);

    /* check the communication with the device reading the ID */
    VL53L4A1_RANGING_SENSOR_ReadID(device, &id);
...
static void MX_53L4A1_MultiSensorRanging_Process(void)
{
  ...
  for (i = 0; i < RANGING_SENSOR_INSTANCES_NBR; i++)
  {
    ...
    VL53L4A1_RANGING_SENSOR_ConfigProfile(i, &Profile);
    status = VL53L4A1_RANGING_SENSOR_Start(i, RS_MODE_BLOCKING_CONTINUOUS);
  ...
  while (1)
  {
    ...
    for (i = 0; i < RANGING_SENSOR_INSTANCES_NBR; i++)
    {
      ...
      status = VL53L4A1_RANGING_SENSOR_GetDistance(i, &Result);
...
static void write_lowpower_pin(uint8_t device, GPIO_PinState pin_state)
{
    switch (device)
    {
      case VL53L4A1_DEV_CENTER:
        HAL_GPIO_WritePin(VL53L4A1_XSHUT_C_PORT, VL53L4A1_XSHUT_C_PIN, pin_state);
        break;

      case VL53L4A1_DEV_LEFT:
        HAL_GPIO_WritePin(VL53L4A1_XSHUT_L_PORT, VL53L4A1_XSHUT_L_PIN, pin_state);
        break;

      case VL53L4A1_DEV_RIGHT:
        HAL_GPIO_WritePin(VL53L4A1_XSHUT_R_PORT, VL53L4A1_XSHUT_R_PIN, pin_state);
        break;
...
static void reset_all_sensors(void)
{
  ...
  HAL_GPIO_WritePin(VL53L4A1_XSHUT_L_PORT, VL53L4A1_XSHUT_L_PIN, GPIO_PIN_RESET);
  HAL_GPIO_WritePin(VL53L4A1_XSHUT_R_PORT, VL53L4A1_XSHUT_R_PIN, GPIO_PIN_RESET);
  HAL_Delay(2);

  HAL_GPIO_WritePin(VL53L4A1_XSHUT_C_PORT, VL53L4A1_XSHUT_C_PIN, GPIO_PIN_SET);
  HAL_GPIO_WritePin(VL53L4A1_XSHUT_L_PORT, VL53L4A1_XSHUT_L_PIN, GPIO_PIN_SET);
  HAL_GPIO_WritePin(VL53L4A1_XSHUT_R_PORT, VL53L4A1_XSHUT_R_PIN, GPIO_PIN_SET);
  HAL_Delay(2);
